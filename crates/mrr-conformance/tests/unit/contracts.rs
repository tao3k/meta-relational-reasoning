...
#[test]
fn external_consumer_materializes_visible_world_closure() {
    let receipt = crate::run_closure_tool(&crate::ClosureToolInput {
        source: "artifact-alpha".into(),
        target: "reviewed-source".into(),
        edges: vec![
            ("artifact-alpha".into(), "build-17".into()),
            ("build-17".into(), "reviewed-source".into()),
            ("artifact-beta".into(), "unreviewed-source".into()),
        ],
    })
    .expect("public facade closure");

    assert!(receipt.reachable);
    assert_eq!(
        receipt.closure.derivations().len(),
