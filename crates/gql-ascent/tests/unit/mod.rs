...
fn derives_transitive_relation_with_snapshot_witness() {
    ...
    let facts = vec![
        gql_reasoning::Fact {
            ...
            values: vec![gql_types::Value::String("A".into()), gql_types::Value::String("B".into())],
        },
        gql_reasoning::Fact {
            predicate: calls,
            values: vec![gql_types::Value::String("B".into()), gql_types::Value::String("C".into())],
        },
    ];
    let result = provider
        .derive(gql_reasoning::DerivationRequest {
            predicate: &gql_catalog::RelationName("DEPENDS_TRANSITIVELY".into()),
            bindings: &[None, None],
            facts: &facts,
            snapshot: "snapshot-1",
    ...
    assert!(result
        .tuples
        .iter()
        .any(|tuple| tuple.values == vec![gql_types::Value::String("A".into()), gql_types::Value::String("C".into())]));
    assert!(result
        .tuples
        .iter()
...
fn derives_transitive_relation_with_truncated_output_when_limit_hits() {
    ...
    let facts = vec![
        ...
        gql_reasoning::Fact {
            predicate: calls,
            values: vec![gql_types::Value::String("A".into()), gql_types::Value::String("D".into())],
        },
    ];
    ...
    assert!(matches!(result.closure, gql_reasoning::ClosureStatus::OutputTruncated));
}
...
fn derives_budget_exhaustion_when_request_exceeds_budget() {
    ...
    assert!(matches!(result.closure, gql_reasoning::ClosureStatus::BudgetExhausted));
}
