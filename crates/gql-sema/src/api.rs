...
use gql_ir::{
    ...
    CatalogCommand, EdgeDirection as IrEdgeDirection, EdgePattern as IrEdgePattern,
    Expression as IrExpression, GraphMatch as IrGraphMatch, GraphMatchMode as IrGraphMatchMode,
    GraphPattern as IrGraphPattern, GraphPatternElement as IrGraphPatternElement, LetBinding,
    NodePattern as IrNodePattern, OptionalMatch as IrOptionalMatch, PathPattern as IrPathPattern,
    PathQuantifier as IrPathQuantifier, ProcedureCommand, QueryBlock,
    SessionCommand as IrSessionCommand, SetOperation as IrSetOperation,
    SetOperator as IrSetOperator, SortKey as IrSortKey, TransactionCommand as IrTransactionCommand,
    UnaryOperator as IrUnaryOperator,
};
use gql_source::{Diagnostic, Span};
use gql_types::ValueType;

use crate::aggregate_analysis::{contains_aggregate, lower_aggregate_call};
use crate::binding_analysis::register_path_bindings;
use crate::data_management::{
    analyze_data_clause, analyze_non_query_statement, lower_declared_value_type,
};
use crate::order_page_semantics::{
    canonical_sort_direction, lower_non_negative_integer_specification, lower_null_ordering,
};
use crate::path_semantics::lower_path_prefix;
use crate::predicate_lowering::{
    label_predicate_operand_is_valid, lower_directed_predicate, lower_element_identity_predicate,
    lower_endpoint_predicate, lower_label_expression, lower_null_predicate,
    lower_property_exists_predicate, lower_truth_predicate,
};
use crate::primitive_query_semantics::analyze_primitive_query_clause;
use crate::query_clause::query_clause_span;
use crate::record_lowering::lower_record_expression;
use crate::result_semantics::analyze_result_clause;
...
pub fn analyze(statement: &Statement, catalog: &dyn GqlCatalog) -> Analysis {
    ...
    for branch in branches {
        ...
        if !branch
            ...
            && !branch
                .iter()
                .any(|clause| matches!(clause, QueryClause::Finish { .. }))
        ...
        match clause {
            QueryClause::Match(match_clause) => {
                for pattern in &match_clause.patterns {
                    binding_order.extend(register_path_bindings(
                        pattern,
                        &mut bindings,
                        diagnostics,
                    ));
                ...
                block
                    .matches
                    .push(build_graph_match(match_clause, &bindings, diagnostics));
            }
            QueryClause::OptionalMatch(match_clause) => {
                ...
                block.optional_matches.push(IrOptionalMatch {
                    graph_match: build_graph_match(match_clause, &bindings, diagnostics),
                    predicate: None,
                });
                pending_optional_match = Some(block.optional_matches.len() - 1);
            }
            QueryClause::Where { expression, span } => {
                if block.matches.is_empty() && block.optional_matches.is_empty() {
                    diagnostics.push(Diagnostic::error(
            ...
            QueryClause::Filter { .. } => {
                analyze_primitive_query_clause(
                    clause,
                    &mut block,
                    &mut bindings,
                    diagnostics,
            ...
            QueryClause::For { item, .. } => {
                ...
                for binding in std::iter::once(&item.binding)
                    .chain(item.ordinality.iter().map(|position| &position.binding))
                {
                    let canonical = binding.canonical_text();
                    if bindings.contains_key(&canonical) && !binding_order.contains(&canonical) {
                        binding_order.push(canonical);
                    }
                }
            }
            QueryClause::Let {
                ...
                for found in found {
                    ...
                    binding_order.push(canonical_binding.clone());
            ...
            QueryClause::Return { .. } | QueryClause::Finish { .. } => {
                unreachable!("result clauses are handled by the result-semantics owner")
            }
            QueryClause::Insert { .. }
            | QueryClause::Set { .. }
            | QueryClause::Remove { .. }
            ...
            QueryClause::Limit { value, span } => {
                if block
                    .limit
                    .replace(lower_non_negative_integer_specification(value))
                    .is_some()
                {
                    diagnostics.push(Diagnostic::error(
                ...
                for key in keys {
                        ...
                        lower_expression(&key.expression, &bindings, diagnostics)
                    {
                        block.order_by.push(IrSortKey {
                            expression,
                            direction: canonical_sort_direction(key.direction),
                            null_ordering: lower_null_ordering(key.null_ordering),
                        });
                    }
                }
            }
...
fn build_graph_match(
    graph_match: &gql_ast::MatchClause,
    bindings: &HashMap<String, ValueType>,
    diagnostics: &mut Vec<Diagnostic>,
) -> IrGraphMatch {
    IrGraphMatch {
        mode: graph_match.mode.map(|mode| match mode {
            gql_ast::GraphMatchMode::RepeatableElements => IrGraphMatchMode::RepeatableElements,
            gql_ast::GraphMatchMode::DifferentEdges => IrGraphMatchMode::DifferentEdges,
        }),
        paths: graph_match
            .patterns
            .iter()
            .map(|path| build_path_pattern(path, bindings, diagnostics))
            .collect(),
        keep: graph_match.keep.as_ref().map(lower_path_prefix),
    }
}
...
fn build_path_pattern(
    ...
    IrPathPattern {
        ...
        prefix: path.prefix.as_ref().map(lower_path_prefix),
        elements: path
            .elements
            .iter()
            .map(|element| build_graph_pattern_element(element, bindings, diagnostics))
...
fn build_graph_pattern_element(
    ...
    match element {
        ...
        PatternElement::Path(path) => {
            IrGraphPatternElement::Path(build_path_pattern(path, bindings, diagnostics))
        }
    }
}
...
pub(crate) fn lower_expression(
    ...
    match expression {
        ...
        Expression::Parameter(parameter) => Some(IrExpression::Parameter(parameter.name.clone())),
        ...
        Expression::FunctionCall { name, span, .. } => {
            diagnostics.push(Diagnostic::error(
        ...
        Expression::AggregateCall {
            ...
            let argument_types = arguments
                .iter()
                .map(|argument| expression_type(argument, bindings).unwrap_or(ValueType::Any))
                .collect::<Vec<_>>();
            let lowered = arguments
                .iter()
                .map(|argument| lower_expression(argument, bindings, diagnostics))
                .collect::<Option<Vec<_>>>()?;
        ...
        Expression::NullPredicate {
            operand, negated, ..
        } => Some(lower_null_predicate(
            lower_expression(operand, bindings, diagnostics)?,
            *negated,
        )),
        ...
        Expression::ValueTypePredicate {
            ...
            negated,
            ..
        } => Some(IrExpression::IsTyped {
            operand: Box::new(lower_expression(operand, bindings, diagnostics)?),
            value_type: lower_declared_value_type(value_type, diagnostics),
            negated: *negated,
        }),
        Expression::DirectedPredicate {
            ...
            negated,
            span,
        } => lower_directed_predicate(
            lower_expression(edge, bindings, diagnostics)?,
            expression_type(edge, bindings).unwrap_or(ValueType::Any),
            *negated,
            *span,
            diagnostics,
        ),
        Expression::EndpointPredicate {
            ...
            negated,
            span,
        } => lower_endpoint_predicate(
            (
                lower_expression(node, bindings, diagnostics)?,
                expression_type(node, bindings).unwrap_or(ValueType::Any),
            ),
            (
                lower_expression(edge, bindings, diagnostics)?,
                expression_type(edge, bindings).unwrap_or(ValueType::Any),
        ...
        Expression::ElementIdentityPredicate {
            ...
            elements,
            span,
        } => lower_element_identity_predicate(
            *kind,
            elements
                .iter()
                .map(|element| {
                    Some((
                        lower_expression(element, bindings, diagnostics)?,
                        expression_type(element, bindings).unwrap_or(ValueType::Any),
                    ))
                })
                .collect::<Option<Vec<_>>>()?,
            *span,
        ...
        Expression::PropertyExistsPredicate {
            element,
            property,
            span,
        } => lower_property_exists_predicate(
            lower_expression(element, bindings, diagnostics)?,
            expression_type(element, bindings).unwrap_or(ValueType::Any),
            property.canonical_text(),
            *span,
            diagnostics,
        ),
        Expression::Binary {
        ...
        Expression::IsLabeled {
            ...
            let operand_ir = lower_expression(operand, bindings, diagnostics)?;
            let operand_type = expression_type(operand, bindings).unwrap_or(ValueType::Any);
            if !label_predicate_operand_is_valid(operand_type, *span, diagnostics) {
                return None;
            }
