...
use mrr_rust_project_harness_policy::MrrRustProjectHarnessEvidenceGraphInput;
use mrr_rust_project_harness_policy::workspace_evidence_graph::{
    MrrRustProjectHarnessWorkspaceEvidenceGraphEdgeKind,
    MrrRustProjectHarnessWorkspaceEvidenceGraphNodeKind,
    MrrRustProjectHarnessWorkspaceEvidenceGraphRequest, build_mrr_workspace_evidence_graph_receipt,
...
fn workspace_receipt_projects_member_crates_and_client_db_graph() {
    ...
    assert!(
        receipt
            .nodes
            .iter()
            .any(|node| node.kind == MrrRustProjectHarnessWorkspaceEvidenceGraphNodeKind::Workspace)
    );
    assert!(
        receipt
            .edges
            .iter()
            .any(|edge| edge.kind == MrrRustProjectHarnessWorkspaceEvidenceGraphEdgeKind::Contains)
    );
}
...
fn default_workspace_receipt_uses_central_member_policy_registry() {
    ...
    assert_eq!(
        ...
        mrr_rust_project_harness_policy::mrr_workspace_member_policies().len()
    );
    assert!(
        receipt
            .members
