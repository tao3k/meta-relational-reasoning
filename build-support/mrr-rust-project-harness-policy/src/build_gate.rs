...
use crate::member_policy::mrr_workspace_member_policy_for;
use std::env;

/// Applies the registered GQL Rust member policy for `package_name` from `build.rs`.
pub fn assert_mrr_rust_project_harness_member_policy_from_env(package_name: &str) {
    let _ = env::var("CARGO_MANIFEST_DIR")
        .unwrap_or_else(|_| panic!("CARGO_MANIFEST_DIR is required to derive default policy"));

    let (harness_config, verification_label) = mrr_workspace_member_policy_for(package_name)
        .map(|member_policy| {
            (
